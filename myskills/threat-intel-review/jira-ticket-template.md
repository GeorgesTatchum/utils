...
Mêmes 4 champs. « Analyse du bug » cadre l'incertitude d'applicabilité (points 4-6 à finaliser après investigation) ; « Raison du blocage/décision » explique pourquoi on investigue avant de remédier.
...
## Variante — ticket de recommandation / amélioration (suivi PSSI)

Pour les recommandations §7 du rapport (process, outillage, gouvernance). **Nature différente d'une remédiation** : pas de CVE, horizon trimestriel, ticket **persistant** rattaché à un Epic d'amélioration continue, pas au ticket de revue mensuel. Objectif : donner à chaque recommandation un porteur et une échéance pour qu'elle **cesse de revenir** faute de visibilité.

Règles :
- **1 recommandation = 1 ticket durable**. Une recommandation **récurrente** garde **un seul** ticket (ne pas recréer chaque mois) : on met à jour son statut et on l'ajoute au suivi §5. Avant d'en créer un, vérifier qu'un ticket standing n'existe pas déjà (ex. `ARCH-VERSIONS-CONVERGENCE`).
- Ne PAS utiliser la structure « 5 pourquoi » (réservée aux failles). Utiliser les champs ci-dessous.
- Rattachement : au **ticket de session du mois `SEC-THREATINTEL-<YYYY-MM>`** (même clé que les tickets de remédiation, ex. CICD-170 pour juin 2026), pas d'Epic dédié. Ce sont malgré tout des Story **persistantes** (elles traversent les mois) : une recommandation récurrente **réutilise son ticket standing** (ex. `ARCH-VERSIONS-CONVERGENCE`) et n'est pas recréée ; on la re-lie à la session du mois où elle est re-suivie.

```
Résumé : [SEC][AMELIORATION][{Thème}] {titre court} (Threat Intel {YYYY-MM})
  Thème ∈ {outillage, process, gouvernance, veille, architecture}
Type : Story / Task (persistante) - rattachée à SEC-THREATINTEL-{YYYY-MM}
Priorité Jira : selon impact (souvent Medium/Low) — PAS de SLA J+n
Horizon : {{trimestre cible, ex. Q3 2026}}
Responsable : {{owner nommé}}
Étiquettes : threat-intel, amelioration, sec-{YYYY-MM}, {thème}

Contexte & objectif :
  {{pourquoi cette action ; quel angle mort / risque elle réduit ; issue de la revue {YYYY-MM} §7}}

Definition of Done :
  * [ ] {{critère concret et vérifiable 1}}
  * [ ] {{critère 2}}
  * [ ] {{artefact / preuve (procédure, capture, config commitée)}}

Bénéfice / risque si non traité :
  {{ce que la reco améliore ; ce qui se passe si elle reste ouverte — typiquement : la reco « revient » chaque mois, angle mort persistant}}

Récurrence / historique :
  {{mois où la recommandation est apparue (ex. mai 2026, juin 2026) — signal de priorisation si elle traîne}}
```