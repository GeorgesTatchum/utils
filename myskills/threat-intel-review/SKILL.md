...
| Snyk SCA | Console interne non publique | `snyk_<moisannée>.csv` |
| GitHub Dependabot | Onglet Security privé | `dependabot_<moisannée>.csv` |
...
## 6ter. Traiter les exports SCA (Dependabot / Snyk) par portée

Les exports SCA peuvent contenir des centaines d'alertes. **Ne pas les agréger en un seul lot P4 ni les reléguer en annexe** : ce sont des items de la revue comme les autres, à trier et prioriser. Le regroupement n'est légitime que lorsque la remédiation est une action unique (comme MSRC ou MariaDB).

**Étape 1 — trier chaque paquet par portée**, critère décisif pour l'exposition :
- Utiliser le flag `dev` du `package-lock.json` de chaque repo : `packages["node_modules/<pkg>"].dev`. Un paquet présent en instance non-`dev` = **runtime** (livré au navigateur de production = exposition internet). Uniquement `dev:true` = **build/dev** (chaîne de build = exposition hors ligne).
- Si le lockfile d'un repo n'est pas disponible localement (ex. repo non cloné), classer par nature du paquet (heuristique) et **signaler "à confirmer"**.

**Étape 2 — évaluer le runtime item par item** : chaque CVE runtime est un item applicable à part entière (exposition internet), priorisé via la matrice §4.4 après récupération du CVSS et de la version corrigée (OSV : `curl -X POST -d '{"package":{"name":"<pkg>","ecosystem":"npm"}}' https://api.osv.dev/v1/query` — l'API par CVE renvoie souvent 404, interroger par package puis filtrer sur l'alias CVE). Croiser la version installée (lockfile) avec la version corrigée pour confirmer la vulnérabilité. Les CVE `@angular/*` runtime sont rattachées à l'item de montée Angular du mois.
- **Exposition conditionnée à une API / feature précise** : quand l'avis (GHSA) indique une précondition d'exploitation (ex. usage du parseur d'expressions `math.evaluate/parse` pour mathjs CWE-915, `_.template` pour lodash, une feature SSR pour Angular), **tracer le chemin de code** avant de prioriser : `grep` des API vulnérables (hors node_modules) + suivre le flux de données. Si la feature vulnérable n'est **pas utilisée**, l'item est **non exploitable dans l'usage actuel** → requalifier en **P4 « montée d'hygiène »** (le composant reste dans la plage affectée et sera remonté par la SCA ; la montée solde le finding et la traçabilité MDR/IEC 62304), et non à la priorité nominale du CVSS. Documenter la conclusion (auteur + date + preuve : grep, flux). Requalifier vers le haut seulement si la feature vulnérable est introduite. Exemple juin 2026 : mathjs 15.1.1 (Hip3D) requalifié P2 → P4 (parseur d'expressions non utilisé, seulement algèbre linéaire numérique).

**Étape 3 — regrouper le build/dev** en lot P4 (exposition hors ligne, remédiation = montée des devDependencies / `npm audit`), en nommant les criticals. Justifier explicitement le regroupement (exposition + remédiation unique), pas par commodité. **Scinder le lot par responsable** quand les repos relèvent de Tech Leads différents (ex. `one-platform` vs les planificateurs Angular) : un sous-item + un ticket par Tech Lead, avec le volume et les criticals propres à chaque groupe. Donner le volume par repo (alertes + criticals) et rappeler que les CVE distinctes se recouvrent entre repos (ne pas additionner les décomptes de CVE distinctes).

**Étape 4 — croiser tout l'export avec le KEV** (garde-fou P1) et **produire une annexe de traçabilité** `annexe_dependabot_<moisannée>.md` listant les CVE distinctes (CVE, package, sévérité, repos, portée), référencée depuis l'item SCA. L'annexe complète la traçabilité par nom, elle ne remplace pas l'évaluation.

Snyk et Dependabot se recouvrent : dédupliquer, mentionner le canal secondaire. Si l'un manque, le noter (couverture SCA partielle) et le porter en rattrapage §6.
